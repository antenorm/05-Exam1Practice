...
def problem0c(circle, n, window):
    ...
    first_center = rg.Point(circle.center.x + (radius * 2), circle.center.y)

    for k in range(n):
        cc = rg.Circle(first_center, radius)
        cc.attach_to(window)
        window.render()
        first_center = rg.Point(first_center.x + (radius * 2), first_center.y)
